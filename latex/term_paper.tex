...
\title{
	\textbf{Application of Multi-Layer Perceptron Neural Network for Predicting Antarctic Sea-Ice Concentration using Meteorological Variables}\\
	\large Term Paper
}
\author{
	Meghraj Goswami\\
...
\begin{document}
	...
	\begin{titlepage}
		...
		\LARGE \textbf{Application of Multi-Layer \\ Perceptron Neural Network for \\ Predicting Antarctic Sea-Ice Concentration using Meteorological Variables} \\[1cm]
	...
	\newpage
	\begin{abstract}
		This project uses feedforward Multi-Layer Perceptron (MLP) Neural Network models for predicting Sea-Ice concentration (SIC) using five key meteorological variables derived from ERA5 reanalysis: sea-surface temperature (SST), 2m air temperature (T2M), 10m u and v wind components (U10, V10) and mean sea-level pressure (MSL). Motivated by the limitations of purely physics-based sea-ice models, the work aims to gauge whether simple neural architectures can capture large-scale sea-ice variability. The MLP models are trained using various combinations of input variables and model hyperparameters, and the performance of the models is evaluated using statistical metrics such as Root Mean Square Error (RMSE), Mean Absolute Error (MAE) and Coefficient of Correlation (R).
		\\\\ The objective of the project is to build and optimize a parsimonious MLP model capable of learning the relationship between atmospheric and oceaning variables and sea-ice concentration, using an exhaustive search over possible network configurations in the defined hyperparameter space. It aims to highlight the potential of MLP models in optimally predicting SIC, which is important for understanding climate dynamics all over the world.
	\end{abstract}
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.75\textwidth]{antarctic_map.jpg}
		\caption{Map of Antarctica showing the 5 sectors}
		\label{fig:map}
	\end{figure}
	...
	\\\\ The Antarctic sea-ice cover is typically divided into five sectors, as depicted in Figure \ref{fig:map}: the Ross Sea, the Weddell Sea, the Indian Ocean sector, the West Pacific sector and the Bellingshausen-Amundsen Seas. Each sector exhibits distinct sea-ice characteristcs due to variations in local atmospheric and oceanic conditions \cite{parkinson2012}.
	...
	\\\\ Antarctica also experiences extreme temperature patterns due to the polar night during the Antarctic winter and experiencing continuous sunlight during the Antarctic summer \cite{temp2}. Depending on the region, temperatures in Antarctica fluctuate between -80°C in winter to 10°C in summer \cite{temp1}. Precipitation is generally low, with most of it falling as snow.
	...
	\end{itemize}
	The target variable for prediction is the Sea-Ice Cover or Sea-Ice Concentration (SIC), a dimensionless quantity representing the fraction of a given area covered by sea-ice, ranging from 0 (no ice) to 1 (complete ice cover). The SIC data is also derived from the ERA5 reanalysis dataset.
	\\\\ The geographical extent of the dataset used in this project is as follows:
	\begin{itemize}
		\item North: -55°
		\item South: -75°
		\item West: -180°
	...
	\begin{itemize}
		\item \textbf{Scripting:} Python 3.13.7 \cite{python}
		...
		\item \textbf{Dataset format:} NetCDF
	\end{itemize}
	All computations are performed on a Linux 6.17.8 system running Arch Linux, with multi-core CPU acceleration enabled during grid search. The Python codebase has been added to a public GitHub repository \cite{github}, as well as in the Appendix section of this report.
	\\\\ A graphical user interface (GUI) has also been developed using DearPyGUI to enable easy and interactive experimentation with different hyperparameter configurations, without modifying the codebase directly. A screenshot of the GUI is shown in Figure \ref{fig:gui}.
	\begin{figure}[h!]
		\centering
	...
	\newpage
	\section{Results}
	To fine-tune the optimal hyperparameters for the MLP Neural Network, an exhaustive grid search is conducted over a user-defined hyperparameter space. The hyperparameters considered in the grid search include:
	...
	Out of the defined hyperparameter space, the best-performing one-layer MLP model based on the evaluation metrics has the following hyperparameters:
	\begin{itemize}
		\item Input Variables: SST, T2M, U10, V10, MSL
	...
	Out of the defined hyperparameter space, the best-performing two-layer MLP model based on the evaluation metrics has the following hyperparameters:
	\begin{itemize}
		\item Input Variables: SST, T2M, U10, V10, MSL
